...
const Post = () => {
  ...
  return (
    <div className='flex gap-[50px] mt-10 flex-col lg:flex-row max-w-[1300px]'>
      <div className="flex-[5] flex flex-col gap-4 border-r border-r-gray-500">

        <div className="content">
          <img src={post.img && require('../uploads/' + post.img)} alt="img" className='w-full h-96 object-contain mb-5 lg:mb-10' />
        </div>
        ...
        <div className="flex items-center gap-4">
         ...
         {currentUser?.username === post.username &&
          <div className="edit flex- flex-col gap-2">
            ...
            <Link to={`/write?edit=${postId}`} state={post}><FaPencilAlt /></Link>
            <span onClick={handleDelete}><ImBin2 /></span>
          </div>}
        </div>

        <div className='mt-4 flex flex-col gap-8'>
          <h2 className='text-4xl font-semibold text-secondaryBlack'>{post?.title}</h2>
