...
export const register = (req, res) => {
  //VALIDATE INPUT
  if (!req.body.email.trim().length || !req.body.email.trim().length)
    return res.status(409).json("Fileds Empty");
  ...
  db.query(q, [req.body.username], (err, data) => {
    ...
    res
      .cookie("access_token", token, {
        //set cookie with name = "access_token" and value is above jwt token
